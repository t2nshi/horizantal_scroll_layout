$(document).ready(function(){
  ...
  $(window).scroll(function(){
    let sPos = $(this).scrollLeft(); //가로 스크롤 값 변수에 담기
    $('.status').html(sPos); //스크롤 값 출력하기
  });
  ...
  $('.gnb li a').click(function(){
    ...
    let secOffset = $(id_value).offset().left; //왼쪽으로 부터 떨어진 값
  ...
  $('main section').each(function(){
    ...
    $(this).on('mousewheel',function(e){
      ...
      if(winEvent.wheelDelta) { //만약에 이벤트에서 휠 데이터 값이 있다면
        delta = winEvent.wheelDelta / 120; //데이터 값을 저장
        if(window.opera) {
            delta = -delta;
        }
      }          
      ...
      // 마우스 휠을 위에서 아래로 이동(처음에서 다음 박스로 이동)
      if(delta < 0) {
      ...
      // 마우스 휠을 아래에서 위로 이동( 뒤에서 앞으로 이동)
      else {
          // 첫번째 article보다 순서 값이 큰 경우에 실행
          if($(this).index() > 0) {
              console.log("▲");
